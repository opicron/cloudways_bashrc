...
alias getgitbash='wget --no-check-certificate --no-cache --no-cookies --no-http-keep-alive https://raw.githubusercontent.com/opicron/cloudways_bashrc/main/.bash_aliases -O ~/.bash_aliases.tmp && rm ~/.bash_aliases; mv ~/.bash_aliases.tmp ~/.bash_aliases && touch ~/.reload_bash'
